...
### August 11, 2026 (generator test: monthly topic self-registration)
- New scripts/test-monthly-topics.py: renders generate-digest.py's update_monthly_archive into a temp dir (real repo untouched; fake month 2026-07 with 2 seeded digest posts + minimal sitemap.xml), runs 6 static assertions on the generated page, injects a fake data-topic="audit" card, and drives the rendered page in Chromium (playwright-core, same dep as xbrowser.js; real assets copied into temp so zero 404 noise): an "Audit" chip must self-register after the known topics with a Title-Case label, filter to the audit card, move aria-pressed, record a tunzua-topic-events entry, and leave zero console errors. Browser layer skips gracefully (exit 3) when node/playwright/chromium are absent; random free port + timeout guard (reviewer fixes)
- Wired in: npm script test:generator + a "Generator test" step in the CI cross-browser job (has npm ci + playwright browsers); smoke job unaffected
- Locally: 6/6 static + 7/7 browser PASS; smoke PASS

### August 11, 2026 (generator test: update_blog_index + trim bug fix)
- New scripts/test-blog-index.py: stdlib-only test running update_blog_index against fixture blog.html pages in a temp dir (repo untouched): prepend (new data-topic=news card lands right after the section marker, href/title/story-count, old cards + marker intact), footer monthly-link rollover (count=1, first occurrence), MAX_LIST_ITEMS trimming (34 digests + 2 evergreens -> exactly 30, oldest 4 removed, boundary kept, evergreens never trimmed, newest first), and the missing-marker RuntimeError guard. 17 checks
- BUG FOUND + FIXED: the digest-trim regex was '<article class="post-card">' (no attributes) but the generator emits '<article class="post-card" data-topic="news">' — trimming had been silently dead since the topic-chips work; the list would have grown unbounded past 30. Regex now '<article class="post-card"[^>]*>' (comment notes the regression test). Live site unaffected (only 4 digest cards today)
- npm test:generator now runs both generator tests; CI cross-browser 'Generator test' step runs both
- Locally: 17/17 PASS + monthly 7/7 PASS via npm run test:generator; smoke PASS

### August 11, 2026 (generator test suite: feed/sitemap, pipeline, quiet-day)
- scripts/test-feed-sitemap.py: update_feed prepend/trim/guard + update_sitemap prepend/trim/guard. Feed: new item lands after the atom:link marker, >MAX items capped newest-first (oldest removed). Sitemap: digest URL prepended before </urlset>, >MAX digest URLs capped (oldest removed, evergreen URLs never touched), missing-marker guards
- scripts/test-pipeline.py: full offline pipeline — monkeypatches fetch() with real RSS XML (parse_items parses for real) and patches ROOT + STATE_FILE to a temp dir, runs main() for one publish day: digest post written, blog.html new card first + footer monthly-link rollover, feed item, sitemap digest + monthly-archive URLs, monthly archive lists the digest, .digest-state.json records 4 guids; second same-day run hits the no-duplicate guard. Seed month derived from the clock (40 days back) so the rollover assertion holds on any date (reviewer fix)
- scripts/test-quiet-day.py: quiet-day path (2 fresh relevant -> 'Quiet news day', exit 0, nothing written), noisy feed (1 fresh + 3 stale + 2 job postings still quiet — proves freshness + job filters), publish threshold (4 items -> publishes), all-feeds-failed -> exit 2, duplicate guard, is_tax_relevant helper checks
- npm test:generator now runs all FIVE generator suites; CI cross-browser 'Generator tests (5 suites)' step runs them all
- Reviewer fixes: seed month dynamic (was hardcoded 2026-07 — would break in July), dropped unused stderr from run_main, hoisted FEED_ITEM_RE constant. Test-side bugs fixed during bring-up: generated feed/sitemap entries are multi-line (assertions updated), re.S flag on the test's own sitemap regex, FATAL message goes to stdout, quiet-day fixture needed the atom:link marker, blog/ reset between sub-tests
- Locally: 5/5 suites PASS (monthly 7/7 incl browser, blog-index 17, feed/sitemap 19, pipeline 15, quiet-day 14); smoke PASS

### August 11, 2026 (test helper, post-template test, daily-digest hardening)
- NEW scripts/_testutil.py: shared scaffolding (REPO, load_generator, check, process-global fails, verdict) — all five generator suites refactored to import from it (duplicated defs removed mechanically with exact-match assertions; monthly keeps its own local fails accumulator for static/browser checks and passes it to verdict explicitly, documented)
- NEW scripts/test-post-template.py (26 checks): build_post_html unit test — head (title, canonical, og:image custom-vs-generic fallback, twitter, JSON-LD BlogPosting with &quot;/&amp; escaping), body (3 digest-items with escaped linked headlines, SOURCE_LABELS + categorize labels, CTA + WhatsApp, how-this-works note, sticky TOC, share row wa/x/in), description truncation with ellipsis, view-count gating by patching GOATCOUNTER_SITE (tracker + span emitted when set, absent when empty), related-posts (present with posts, digests first, today excluded, capped at 3; asserted on the section markup because the string 'related-posts' also lives in the TOC JS)
- .github/workflows/daily-digest.yml hardened: concurrency group (no overlapping runs), a pre-publish 'Sanity check generator' step running all six offline suites (a broken generator edit now fails BEFORE writing a bad commit), commit+push pushes origin main EXPLICITLY with a 3-attempt retry loop and a final git merge-base --is-ancestor HEAD origin/main verification that exits 1 loudly if the digest commit never lands (the user's 'push fails mid-way' gap), and Summarize is if: always() so failures stay diagnosable. Retry logic validated end-to-end in a sandbox git repo (failure path: 3 retries + loud error; success path: pushed + confirmed; quiet path: skipped)
...
### August 11, 2026 (evergreen content + local SEO + Search Console prep)
- Added three SEO evergreen posts (blog/gst-registration-cost-india, blog/udyam-msme-registration-benefits, blog/tally-prime-price-india), generated from the existing post chrome (nav/footer/scripts/JSON-LD reused) with fact-checked content: GST registration is free (professional fees ₹500-10k; thresholds ₹40L/₹20L goods, ₹20L services, special-category states incl. Manipur/Mizoram ₹20L both per Notification 10/2019); MSME/Udyam micro/small/medium limits + CGTMSE/late-payment benefits; Tally Silver ₹22,500 / Gold ₹67,500 + 18% GST, rentals, TSS renewal. All dated 2026-08-11 consistently (post, JSON-LD, blog card, sitemap lastmod, feed pubDate)
- Registered them: blog.html cards (data-topic gst/msme/bookkeeping), sitemap.xml (+3 URLs, 17 total), feed.xml (+3 items); added msme:'MSME' to the topic-chip labels in blog.html, blog/monthly/2026-08.html and scripts/generate-digest.py so the new topic displays cleanly
- Local SEO: LocalBusiness schema on index.html gained geo (Churachandpur 24.3341, 93.6896), sameAs (facebook/x/instagram/youtube), image+logo — all verifiable public data, nothing invented (opening hours intentionally left out pending real hours from the firm)

### August 11, 2026 (evergreen batch 2: ITR self-filing, TDS, business structures)
- Three more SEO evergreen posts, generated from the existing post chrome via a throwaway script (build_evergreens2.py):
  - blog/how-to-file-income-tax-return-yourself-india — ITR-1 eligibility (resident, ≤₹50L, one house property), documents (Form 16, 26AS/AIS), portal steps, verification (Aadhaar OTP/EVC/DSC/ITR-V), costs (portal free; 234F late fee ₹1k/₹5k) and AY 2026-27 deadlines (31 Jul salaried, 31 Aug business, 31 Dec belated/revised)
  - blog/tds-rates-and-due-dates-2026-27 — rates table (194C 1%/2% · 194J 10%/2% · 194I 2%/10% · 194A 10% · 194H 5% · 194D 5% · 194B 30% · new 194T 10% partners from 1 Apr 2025), deposit by 7th (March by 30 Apr, 1.5%/month late interest), quarterly returns 31 Jul/31 Oct/31 Jan/31 May, Form 16/16A; notes Income-tax Act 2025 consolidates into Section 393
